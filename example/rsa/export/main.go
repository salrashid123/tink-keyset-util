...
var (
	insecureKeySetFile = flag.String("insecure-key-set", "/tmp/rsa_1_private.bin", "Parse a cleartext keyset")
	pubout             = flag.String("pubout", "/tmp/rsa_1_public.pem", "Parse a cleartext keyset")
	privout            = flag.String("privout", "/tmp/rsa_1_private.pem", "Parse a cleartext keyset")
)
...
func main() {
	...
	if ku.GetKeySetTypeURL() == keysetutil.RsaSsaPkcs1VerifierTypeURL {
		...
		if ku.GetKeySetTypeURL() == keysetutil.RsaSsaPkcs1VerifierTypeURL {
			...
			pubpem := pem.EncodeToMemory(publicKeyPEM)
			fmt.Printf("RSA PublicKey: \n%s\n", pubpem)

			err = os.WriteFile(*pubout, []byte(pubpem), 0644)
			if err != nil {
				log.Fatal(err)
			}
		...
		if ku.GetKeySetTypeURL() == keysetutil.RsaSsaPkcs1PrivateKeyTypeURL {
			...
			fmt.Printf("RSA Key: \n%s\n", string(pemdata))
			err = os.WriteFile(*privout, []byte(pemdata), 0644)
			if err != nil {
				log.Fatal(err)
			}
